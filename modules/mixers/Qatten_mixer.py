...
class QattenMixer(nn.Module):
    def __init__(self, args,cent_obs_dim, num_agents, device, ppo=False):
        ...
        self.state_dim = args.obs_shape * self.n_agents
        self.unit_dim = args.unit_dim
        self.n_actions = args.n_actions
        self.sa_dim = self.state_dim + self.n_agents * self.n_actions
        self.n_head = args.n_heads  # attention head num
        self.state_bias = args.state_bias
        self.nonlinear_key = args.nonlinear_key
        ...
        if getattr(args, "hypernet_layers", 1) == 1:
            ...
            for i in range(self.n_head):  # multi-head attention
                selector_nn = nn.Sequential(nn.Linear(self.state_dim, self.hypernet_hidden_dim),
                                            nn.ReLU(),
                                            nn.Linear(self.hypernet_hidden_dim, self.embed_dim, bias=False))
                ...
                if self.nonlinear_key:  # add qs
                    ...
                    self.key_extractors.append(nn.Linear(self.unit_dim, self.embed_dim, bias=False))  # key
            if self.weighted_head:
                self.hyper_w_head = nn.Sequential(nn.Linear(self.state_dim, self.hypernet_hidden_dim),
                                                  nn.ReLU(),
                                                  nn.Linear(self.hypernet_hidden_dim, self.n_head))
        elif getattr(args, "hypernet_layers", 1) > 2:
    ...
    def forward(self, agent_qs, states=None, actions=None):
        bs = agent_qs.size(0)
        ...
        if self.nonlinear_key:
            unit_states = th.cat((unit_states, agent_qs.permute(2, 0, 1)), dim=1)
        ...
        head_entropies = [(-((probs + 1e-8).log() * probs).squeeze(dim=1).sum(1).mean()) for probs in
                          head_attend_weights]
        return q_tot.repeat(1,1,self.n_agents), attend_mag_regs, head_entropies