...
import pufferlib
from src_minigrid.envs.PufferLib_2.pufferlib import namespace
from src_minigrid.envs.PufferLib_2.pufferlib.emulation import GymnasiumPufferEnv, PettingZooPufferEnv
from src_minigrid.envs.PufferLib_2.pufferlib.environment import PufferEnv, set_buffers
from src_minigrid.envs.PufferLib_2.pufferlib.exceptions import APIUsageError
from src_minigrid.envs.PufferLib_2.pufferlib.namespace import Namespace
import src_minigrid.envs.PufferLib_2.pufferlib.spaces
import gymnasium
...
def get_obs(vecenv):
     vecenv.async_get_obs() #change things in recv
     obs = vecenv.recv()
     return  obs

def get_avail_actions(vecenv,self):
    return vecenv.get_avail_actions()

def get_state(vecenv):
    return vecenv.get_state()

def get_carryings(vecenv):
    return vecenv.carryings

def open_doors(vecenv):
    return vecenv.doors_opened

def euc_dist(vecenv):
    return dist.euclidean(vecenv.agent_posits[0], vecenv.agent_posits[1])
...
class Serial:
    ...
    def __init__(self, env_creators, env_args, env_kwargs, num_envs, buf=None, **kwargs):
        ...
        self.agents_per_batch = self.driver_env.n_agents * num_envs
        ...
        set_buffers(self, buf, serial=1)

        self.envs = []
        ptr = 0
        for i in range(num_envs):
            end = ptr + self.driver_env.n_agents
            buf_i = namespace(
...
def _worker_process(env_creators, env_args, env_kwargs, obs_shape, obs_dtype, atn_shape, atn_dtype,
    ...
    while True:
        ...
        if sem == RESET:
            ...
            _, _, _, _, infos = envs.reset(seed=seeds)
        elif sem == STEP:
...
class Multiprocessing:
    ...
    def __init__(self, env_creators, env_args, env_kwargs,
        ...
        self.num_agents = num_agents = driver_env.n_agents * num_envs
        self.agents_per_batch = driver_env.n_agents * batch_size
        agents_per_worker = driver_env.n_agents * envs_per_worker
        ...
        self.shm = namespace(
            ...
            aval=RawArray(atn_ctype, num_agents * int(np.prod(atn_shape))),
        ...
        self.buf = namespace(
            ...
            aval=np.ndarray(shape, dtype=int, buffer=self.shm.aval),
            dist=np.ndarray(shape, dtype=bool, buffer=self.shm.dist),
            key=np.ndarray(shape, dtype=bool, buffer=self.shm.key),
        )
        ...
        for i in range(num_workers):
            ...
            p = Process(
                ...
                args=(env_creators[start:end], env_args[start:end],
                    ...
                    atn_shape, atn_dtype, envs_per_worker, driver_env.n_agents,
    ...
    def recv(self):
        ...
        o = buf.observations[w_slice].ravel()
    ...
    def async_get_obs(self):
        self.flag = RECV
        self.buf.semaphores[:]  = OBS
        self.flag = RECV
        obs = []
